...
const uniquePermissions = (permissions) => {
  ...
  permissions.forEach((permission) => {
    ...
    if (permissionName) permissionMap.set(permissionName, permission);
  });
...
const DetailRole = () => {
  ...
  const rawPermissions = useMemo(() => {
    ...
    const relationList = Array.isArray(rolePermissionsData)
      ? rolePermissionsData
      : rolePermissionsData?.data || [];
    const permissionsFromRelations = relationList
      .filter((item) => getRelationRoleId(item) === roleId)
      .map(extractPermission);
    const permissionsFromRole = Array.isArray(roleData?.permissions)
      ? roleData.permissions
      : [];

    return uniquePermissions([...permissionsFromRelations, ...permissionsFromRole]);
  }, [id, roleData, rolePermissionsData]);
  ...
  const groupedPermissions = useMemo(() => {
    ...
    const filtered = activePermissions.filter((p) => {
      const pName = getPermissionName(p);
